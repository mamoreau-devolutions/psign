...
//!
//! When the executable is invoked as `signtool-windows @path`, the file is read: one argument per
...
pub fn expand_invocations(executable: OsString, tail: Vec<OsString>) -> Result<Vec<Vec<OsString>>> {
    // Classic: `signtool-windows @file` — multi-block response file (blank line between commands).
    if tail.len() == 1
...
mod tests {
    ...
    fn expands_at_file_to_extra_args() {
        ...
        let inner = dir.join("signtool_rs_rsp_inner.txt");
        fs::write(&inner, "/pa\n").expect("write inner");
        let rsp = dir.join("signtool_rs_rsp_outer.txt");
        fs::write(&rsp, format!("verify\n@{}\nx.exe\n", inner.display())).expect("write rsp");

        let exe = OsString::from("signtool-windows");
        ...
        assert_eq!(
            inv[0]
                ...
                .collect::<Vec<_>>(),
            vec!["signtool-windows", "verify", "/pa", "x.exe",]
        );
    ...
    fn at_only_form_two_command_blocks() {
        let dir = std::env::temp_dir();
        let rsp = dir.join("signtool_rs_rsp_multiblock.txt");
        fs::write(&rsp, "verify\n/pa\na.exe\n\nsign\n/fd\nSHA256\nb.exe\n").expect("write");
        ...
        assert_eq!(
            inv[0]
                ...
                .map(|s| s.to_string_lossy().to_string())
                .collect::<Vec<_>>(),
            vec!["signtool-windows", "verify", "/pa", "a.exe",]
        );
        assert_eq!(
            inv[1]
                .iter()
                .map(|s| s.to_string_lossy().to_string())
                .collect::<Vec<_>>(),
            vec!["signtool-windows", "sign", "/fd", "SHA256", "b.exe",]
        );
        let _ = fs::remove_file(&rsp);
    }
    ...
    fn response_file_quoted_path_single_argument() {
        let dir = std::env::temp_dir();
        let spaced = dir.join("signtool_rs_rsp spaced target.exe");
        fs::write(&spaced, b"0").expect("write target");
        let rsp = dir.join("signtool_rs_rsp_quoted_path.txt");
        fs::write(
            &rsp,
    ...
    fn response_file_quoted_include_path() {
        ...
        fs::write(&inner, "/pa\n").expect("write inner");
        let rsp = dir.join("signtool_rs_rsp_quote_include.txt");
        fs::write(
            &rsp,
    ...
    fn response_file_utf16_le_bom_roundtrip() {
        let dir = std::env::temp_dir();
        let rsp = dir.join("signtool_rs_rsp_utf16le_bom.rsp");
        let text = "verify\n/pa\nx.exe\n";
    ...
    fn response_file_utf16_be_bom_roundtrip() {
        let dir = std::env::temp_dir();
        let rsp = dir.join("signtool_rs_rsp_utf16be_bom.rsp");
        let text = "verify\n/pa\ny.exe\n";
    ...
    fn response_file_utf8_bom_roundtrip() {
        let dir = std::env::temp_dir();
        let rsp = dir.join("signtool_rs_rsp_utf8_bom.rsp");
        let body = "verify\n/pa\nz.exe\n";
