    ...
    fn open_msix_subject(
        ...
        match unsafe {
            ...
            Ok(h) => {
                if debug {
                    eprintln!(
                        "[signtool-rs debug] msix CreateFileW returned unusable handle {:?} for {}",
                        h,
                        target.display()
                    );
                }
            ...
            Err(e) => {
                if debug {
                    eprintln!(
                        "[signtool-rs debug] msix CreateFileW failed for {}: {}",
                        target.display(),
                        e
                    );
                }
...
fn log_sign_format(target: &std::path::Path, global: &GlobalOpts) {
    ...
    eprintln!(
        "[signtool-rs debug] sign target format={fmt:?} sip_hint={}",
        fmt.sip_hint()
    );
}
...
pub fn sign_with_mssign32(
    ...
    if global.debug && msix_family {
        eprintln!(
            "[signtool-rs debug] msix SIGNER_FILE_INFO.hFile={:?} is_invalid={}",
            file_info.hFile,
