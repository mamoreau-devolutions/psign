...
- `msix-minimal/AppxManifest.xml` — Identity publisher `CN=Test Code Signing Certificate` (matches Devolutions test signing cert). CI copies `target/debug/signtool-rs.exe` as `noop.exe` and adds `Assets/StoreLogo.png` before `MakeAppx pack`.
...
## Linux Authenticode trust (`signtool-digest trust-verify-pe`)
