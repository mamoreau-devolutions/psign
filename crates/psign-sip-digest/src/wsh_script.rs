...
mod tests {
    ...
    #[test]
    fn wsh_digest_unsigned_errors_missing_markers() {
        let raw = b"// no sig markers\nconsole.log(1);\n";
        let err = verify_wsh_digest_consistency(raw, "js").unwrap_err();
        let msg = format!("{err:#}");
        assert!(
            msg.contains("marker") || msg.contains("Begin") || msg.contains("PKCS"),
            "unexpected: {msg}"
        );
    }
}
