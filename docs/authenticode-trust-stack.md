...
1. **`--as-of YYYY-MM-DD`** (CLI) / **`verification_instant_override`** — fixed UTC midnight for reproducible CI or expired-leaf fixtures (**timestamp presence is not checked** on this path; without **`--as-of`**, **`--require-valid-timestamp`** still succeeds when extraction finds a nested **`TSTInfo.genTime`** or PKCS#9 **`signing-time`**).
2. Otherwise, if **`prefer_timestamp_signing_time`** is **false** (default): wall clock **`UtcDate::now()`**.
3. If **`prefer_timestamp_signing_time`** is **true**: **[`rfc3161_extract::utc_date_from_authenticode_timestamp_token`](../crates/psign-authenticode-trust/src/rfc3161_extract.rs)** scans CMS **`SignedData`** **`SignerInfo`** rows — **first** nested PKCS#7 with **`id-ct-TSTInfo`** encapsulated content and a parsable **`TSTInfo.genTime`** wins; if none, the **first** PKCS#9 **`signing-time`** in signed attributes wins. If still none and **`require_valid_timestamp`** is set, verification fails; if **`require_valid_timestamp`** is unset, falls back to wall clock.
