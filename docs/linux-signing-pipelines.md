...
2. **`:sign` LRO** (same as **`psign-tool-windows artifact-signing-submit`**):
...
3. **Embed** PKCS#7 / complete Authenticode: still **`psign-tool-windows`** + **`SignerSignEx3`** (and typically **`--dlib`** / **`--dmdf`** for Trusted Signing) until a portable embedder exists.
...
**Embed** PKCS#7 on Windows with **`psign-tool-windows`** (`--features azure-kv-sign`) or native **`signtool.exe`**.
