...
This document summarizes native `signtool.exe` options vs the **`psign-tool-windows`** CLI (Rust package **`psign`**). The **machine-readable source of truth** is [`psign-cli-matrix.json`](psign-cli-matrix.json) (`commands.sign`, `commands.verify`, `commands.timestamp`, `commands.catdb`, `commands.remove`, `global_options`, `invocation`, `code_sign_file_formats`).
