...
|-------|----------|-------------|
| PE digest vs PKCS#7 indirect digest | [`crates/signtool-sip-digest/src/verify_pe.rs`](crates/signtool-sip-digest/src/verify_pe.rs) | Yes — **integrity only** |
| CMS `SignedData` parsing (catalogs) | [`crates/signtool-sip-digest/src/catalog_digest.rs`](crates/signtool-sip-digest/src/catalog_digest.rs) | Yes — **`messageDigest` vs `eContent`** |
...
Understanding this avoids guessing; “decompile” is unnecessary — trace **`signtool-rs`** Win32 paths and public docs.
...
2. **Phase B:** Parse **`authrootstl.stl`** CTL directly from CAB — reuse **`cms`** patterns from [`catalog_digest.rs`](crates/signtool-sip-digest/src/catalog_digest.rs) (also Microsoft CTL content type) to decode **`SignedData`**, extract **`eContent`**, then ASN.1-parse CTL body with **picky / picky-asn1** generated or hand-written models (may need a thin CTL schema crate or internal module).
...
| TOFU store in `~/.cache/signtool-rs/authroot/` | UX | Still needs first-fetch integrity |
...
| CMS **`SignedData`**, **`SignerInfo`**, digest alg | `cms`, `der` | Matches [`catalog_digest.rs`](crates/signtool-sip-digest/src/catalog_digest.rs) patterns |
...
### 6.3 Alignment with `signtool-rs` Windows path
...
- **`crates/signtool-authenticode-trust`** — **recommended**: isolates **picky** + trust logic; **`signtool-sip-digest`** stays digest-focused; **`signtool-digest-cli`** gains subcommands.
- Feature flag **`signtool-sip-digest/trust`** — avoids new crate but couples digest + PKI dependency weight.
