...
**Page-hash** authenticated attributes (`1.3.6.1.4.1.311.2.3.1` / `.2`) carry a separate flat table of **`(end_offset, digest)`** pairs. Native **`WinVerifyTrust`** `/ph` validates those entries against PE bytes with rules that **do not** reduce to “hash contiguous raw slices from offset 0”. The portable CLI command **`signtool-portable verify-pe-page-hashes`** implements an **experimental contiguous raw-file model** only — closing Tier 1c parity requires matching **`WINTRUST`/`CryptSIP`** page-boundary and exclusion semantics (or reusing their outputs via FFI on Windows).
