...
| **signtool-rs** | Rust port (this repo): Windows build uses **SignerSignEx3** / **WinVerifyTrust** / OS SIPs; digest helpers mirror inbox SIP hashing where implemented in **`signtool-sip-digest`**. |
...
Rough alignment between inbox SIP and **`signtool-sip-digest`** / **`--rust-sip`** checks:
