...
| Item | Status |
|------|--------|
| PE **PKCS#7 encode** + **`WIN_CERTIFICATE`** embed entirely in Rust | Stubbed (`pe_embed.rs`); `pkcs7.rs` parses **`SpcIndirectDataContent`** from embedded PE PKCS#7 (**`parse_pe_pkcs7_spc_indirect_data_at`** / **`parse_pe_pkcs7_spc_indirect_data`** for index **`0`**), replaces **`messageDigest`** (**`spc_indirect_data_replace_message_digest`**), and regression-tests encode/decode on **`tiny32`** / **`tiny64`** fixtures. **`signtool-portable`** can inspect/extract PKCS#7 rows by index for multi-signed binaries. Full **`SignedData`** / **`SignerInfo`** production still OS-delegated. |
