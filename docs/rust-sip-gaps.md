...
| Item | Status |
|------|--------|
| PE **PKCS#7 encode** + **`WIN_CERTIFICATE`** embed entirely in Rust | **`pe_embed`**: **`wrap_pkcs7_der_authenticode_win_certificate`** + **`pe_append_authenticode_pkcs7_certificate`** (grow attribute cert table, patch security directory, **`pe_compute_image_checksum`**). **`pkcs7.rs`**: PKCS#9 **`messageDigest`** extraction (**`signer_info_pkcs9_message_digest_octets`**), **`encapContentInfo.eContent`** digest (**`cms_digest_encapsulated_econtent_bytes`**), authenticated-attribute **`SET OF Attribute`** DER (**`signer_info_signed_attributes_sequence_der`** — **RFC 5652** §5.4); parse/replace **`SpcIndirectDataContent`**, **`encode_pkcs7_content_info_signed_data_der`**, **`signed_data_replace_encapsulated_spc_indirect`**. Full **CMS `SignerInfo` re-sign** (attrs + **`encryptedDigest`**) + unsigned-first-sign workflow still OS-delegated / missing. |
