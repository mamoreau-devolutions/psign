...
| Item | Status |
|------|--------|
| PE **PKCS#7 encode** + **`WIN_CERTIFICATE`** embed entirely in Rust | Stubbed (`pe_embed.rs`); `pkcs7.rs` now regression-tests **`SpcIndirectDataContent`** **`der` decode → encode → decode** parity on **`tiny32.signed.efi`** (canonical DER may differ from Microsoft’s bytes; semantics + embedded SHA-256 digest match Rust SIP). Full **`SignedData`** / **`SignerInfo`** production still OS-delegated. |
