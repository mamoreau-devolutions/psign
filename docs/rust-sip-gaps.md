...
| Item | Status |
|------|--------|
| PE **PKCS#7 encode** + **`WIN_CERTIFICATE`** embed entirely in Rust | Stubbed (`pe_embed.rs`); `pkcs7.rs` parses **`SpcIndirectDataContent`** from embedded PE PKCS#7 (**`parse_pe_pkcs7_spc_indirect_data`**), replaces **`messageDigest`** (**`spc_indirect_data_replace_message_digest`**), and regression-tests encode/decode on **`tiny32`** / **`tiny64`** fixtures. Full **`SignedData`** / **`SignerInfo`** production still OS-delegated. |
