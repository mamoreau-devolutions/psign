...
| Item | Status |
|------|--------|
| PE **PKCS#7 encode** + **`WIN_CERTIFICATE`** embed entirely in Rust | **`pe_embed`**: **`wrap_pkcs7_der_authenticode_win_certificate`** + **`pe_append_authenticode_pkcs7_certificate`** (grow attribute cert table, patch security directory, **`pe_compute_image_checksum`**). **`pkcs7.rs`** parses/replaces **`SpcIndirectDataContent`**; **`signtool-portable`** inspect/extract/index/append PKCS#7 rows. Full **CMS `SignedData` / `SignerInfo` producer** + unsigned-first-sign workflow still OS-delegated / missing. |
