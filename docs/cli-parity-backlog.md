...
These items are **native `signtool.exe` switches and workflows** that are intentionally partial or stubbed in `signtool-rs`. They are independent of **file-format / SIP** coverage: signing PE, WinMD, MSIX, scripts, etc. still goes through the OS Cryptography SIP stack (`SignerSignEx3`, `WinVerifyTrust`) where implemented.

Source of truth for tiers and per-flag notes: [`signtool-cli-matrix.json`](signtool-cli-matrix.json).
