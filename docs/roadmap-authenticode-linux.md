...
- **Local Linux/macOS mirror of CI** (from repo root, `--locked` optional but matches CI):  
  `cargo test -p psign-sip-digest --lib --locked`, `cargo test -p psign-authenticode-trust --lib --locked`, and `cargo test -p psign --test cli_pe_digest --locked`, or `cargo digest-test`.
