...
| PKCS#7 builder / WIN_CERTIFICATE embed in Rust | partial | `pkcs7.rs`: parse/replace **`SpcIndirectDataContent`** + SIP digest checks; **`pe_embed.rs`**: **`wrap_pkcs7_der_authenticode_win_certificate`**, **`pe_append_authenticode_pkcs7_certificate`** (append PKCS#7 row; unit-tested on **`tiny32`**); **`pkcs7_wire`**: **`pkcs7_outer_sequence_prefix`** trims DER vs padding; **`signtool-portable`** extract/list/inspect by index; CMS **`SignedData`** production + checksum / full sign pipeline still todo; architecture in `docs/rust-sip-architecture.md` |
