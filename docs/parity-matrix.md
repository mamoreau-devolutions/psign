...
| PKCS#7 builder / WIN_CERTIFICATE embed in Rust | partial | `pkcs7.rs`: **`SpcIndirectDataContent`** ASN.1 round-trip + digest-vs-SIP unit test on **`tiny32.signed.efi`**; **`SignedData`** signing + `pe_embed.rs` **`WIN_CERTIFICATE`** growth still todo; architecture in `docs/rust-sip-architecture.md` |
