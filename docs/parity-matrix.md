...
| PKCS#7 builder / WIN_CERTIFICATE embed in Rust | partial | `pkcs7.rs`: parse PE PKCS#7 **`SpcIndirectDataContent`**, **`messageDigest`** replace helper, ASN.1 round-trip + SIP digest checks on **`tiny32`** / **`tiny64`**; **`SignedData`** signing + `pe_embed.rs` **`WIN_CERTIFICATE`** growth still todo; architecture in `docs/rust-sip-architecture.md` |
