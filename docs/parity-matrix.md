...
| PKCS#7 builder / WIN_CERTIFICATE embed in Rust | partial | `pkcs7.rs`: parse PE PKCS#7 **`SpcIndirectDataContent`** (**`parse_pe_pkcs7_spc_indirect_data_at`** / index **`0`** helper), **`messageDigest`** replace helper, ASN.1 round-trip + SIP digest checks on **`tiny32`** / **`tiny64`**; **`signtool-portable`** **`extract-pe-pkcs7`**, **`list-pe-pkcs7`**, **`inspect-pe-spc-indirect`** share indexed PKCS#7 row order; **`SignedData`** signing + `pe_embed.rs` **`WIN_CERTIFICATE`** growth still todo; architecture in `docs/rust-sip-architecture.md` |
