...
function Resolve-SignTool {
    ...
    $dest = Join-Path $env:TEMP "signtool_rs_parity_native.exe"
...
$parityAtRsp = Join-Path $env:TEMP "signtool_rs_parity_at_verify.rsp"
...
$parityAtQuotedRsp = Join-Path $env:TEMP "signtool_rs_parity_at_verify_quoted.rsp"
...
$parityAtUtf16Rsp = Join-Path $env:TEMP "signtool_rs_parity_at_verify_utf16.rsp"
...
foreach ($scenario in $scenarios) {
    ...
    # signtool-rs decodes UTF-16 per response_argv.rs — exit divergence is expected, not a Rust regression.
...
$tmpRmParity = Join-Path $env:TEMP "signtool_rs_remove_noflags_parity.exe"
...
$tmpRmMulti1 = Join-Path $env:TEMP "signtool_rs_remove_multi1.exe"
$tmpRmMulti2 = Join-Path $env:TEMP "signtool_rs_remove_multi2.exe"
...
$tmpRmQuiet = Join-Path $env:TEMP "signtool_rs_remove_quiet.exe"
...
$tmpRmUNative = Join-Path $env:TEMP "signtool_rs_remove_u_native.exe"
$tmpRmURust = Join-Path $env:TEMP "signtool_rs_remove_u_rust.exe"
...
$tmpRmCNative = Join-Path $env:TEMP "signtool_rs_remove_c_native.exe"
$tmpRmCRust = Join-Path $env:TEMP "signtool_rs_remove_c_rust.exe"
...
$tmpRmCUNative = Join-Path $env:TEMP "signtool_rs_remove_cu_native.exe"
$tmpRmCURust = Join-Path $env:TEMP "signtool_rs_remove_cu_rust.exe"
...
if ($env:SIGNTOOL_RS_SIGNED_FIXTURE -and $env:SIGNTOOL_RS_TIMESTAMP_URL) {
    ...
    $tmpTs1 = Join-Path $env:TEMP "signtool_rs_ts_tr_two_1.exe"
    $tmpTs2 = Join-Path $env:TEMP "signtool_rs_ts_tr_two_2.exe"
...
if ($env:SIGNTOOL_RS_UNSIGNED_FIXTURE -and $env:SIGNTOOL_RS_TEST_PFX) {
    ...
    $tempSigned = Join-Path $env:TEMP "signtool_rs_ci_semantic.exe"
    ...
    $tmpSignA = Join-Path $env:TEMP "signtool_rs_sign_two_a.exe"
    $tmpSignB = Join-Path $env:TEMP "signtool_rs_sign_two_b.exe"
    ...
    $tmpPeNat = Join-Path $env:TEMP ("signtool_rs_pe_fixture_native" + $peExt)
    $tmpPeRust = Join-Path $env:TEMP ("signtool_rs_pe_fixture_rust" + $peExt)
    ...
    $tmpDesc = Join-Path $env:TEMP "signtool_rs_verify_desc.exe"
    Copy-Item -LiteralPath $env:SIGNTOOL_RS_UNSIGNED_FIXTURE -Destination $tmpDesc -Force
    $parityDesc = "signtool_rs_parity_desc_2026"
    $parityUrl = "https://example.invalid/signtool-rs-parity"
    ...
    if (Test-Path -LiteralPath $ps1Src) {
        ...
        $tmpPs1Nat = Join-Path $env:TEMP "signtool_rs_ps1_native.ps1"
        $tmpPs1Rust = Join-Path $env:TEMP "signtool_rs_ps1_rust.ps1"
        ...
        $tmpPs1DescNat = Join-Path $env:TEMP "signtool_rs_verify_desc_native.ps1"
        $tmpPs1DescRust = Join-Path $env:TEMP "signtool_rs_verify_desc_rust.ps1"
    ...
    if (Test-Path -LiteralPath $psm1Src) {
        ...
        $tmpPsm1Nat = Join-Path $env:TEMP "signtool_rs_psm1_native.psm1"
        $tmpPsm1Rust = Join-Path $env:TEMP "signtool_rs_psm1_rust.psm1"
        ...
        $tmpPsm1DescNat = Join-Path $env:TEMP "signtool_rs_verify_desc_native.psm1"
        $tmpPsm1DescRust = Join-Path $env:TEMP "signtool_rs_verify_desc_rust.psm1"
    ...
    if (Test-Path -LiteralPath $psd1Src) {
        ...
        $tmpPsd1Nat = Join-Path $env:TEMP "signtool_rs_psd1_native.psd1"
        $tmpPsd1Rust = Join-Path $env:TEMP "signtool_rs_psd1_rust.psd1"
        ...
        $tmpPsd1DescNat = Join-Path $env:TEMP "signtool_rs_verify_desc_native.psd1"
        $tmpPsd1DescRust = Join-Path $env:TEMP "signtool_rs_verify_desc_rust.psd1"
    ...
    if (Test-Path -LiteralPath $jsSrc) {
        ...
        $tmpJsNat = Join-Path $env:TEMP "signtool_rs_js_native.js"
        $tmpJsRust = Join-Path $env:TEMP "signtool_rs_js_rust.js"
        ...
        $tmpJsDescNat = Join-Path $env:TEMP "signtool_rs_verify_desc_native.js"
        $tmpJsDescRust = Join-Path $env:TEMP "signtool_rs_verify_desc_rust.js"
    ...
    if (Test-Path -LiteralPath $vbsSrc) {
        ...
        $tmpVbsNat = Join-Path $env:TEMP "signtool_rs_vbs_native.vbs"
        $tmpVbsRust = Join-Path $env:TEMP "signtool_rs_vbs_rust.vbs"
        ...
        $tmpVbsDescNat = Join-Path $env:TEMP "signtool_rs_verify_desc_native.vbs"
        $tmpVbsDescRust = Join-Path $env:TEMP "signtool_rs_verify_desc_rust.vbs"
    ...
    if (Test-Path -LiteralPath $wsfSrc) {
        ...
        $tmpWsfNat = Join-Path $env:TEMP "signtool_rs_wsf_native.wsf"
        $tmpWsfRust = Join-Path $env:TEMP "signtool_rs_wsf_rust.wsf"
        ...
        $tmpWsfDescNat = Join-Path $env:TEMP "signtool_rs_verify_desc_native.wsf"
        $tmpWsfDescRust = Join-Path $env:TEMP "signtool_rs_verify_desc_rust.wsf"
...
if ($env:SIGNTOOL_RS_MSIX_UNSIGNED_FIXTURE -and $env:SIGNTOOL_RS_MSIX_TEST_PFX -and $env:SIGNTOOL_RS_MSIX_TIMESTAMP_URL) {
    ...
    $tempNativeMsix = Join-Path $env:TEMP "signtool_rs_ci_msix_native.msix"
    $tempRustMsix = Join-Path $env:TEMP "signtool_rs_ci_msix_rust.msix"
    ...
    $msixParityDesc = "signtool_rs_parity_desc_2026"
    $msixParityUrl = "https://example.invalid/signtool-rs-parity"
    $tempNativeMsixDesc = Join-Path $env:TEMP "signtool_rs_ci_msix_native_desc.msix"
    $tempRustMsixDesc = Join-Path $env:TEMP "signtool_rs_ci_msix_rust_desc.msix"
...
if ($env:SIGNTOOL_RS_MSIX_UNSIGNED_FIXTURE -and $env:SIGNTOOL_RS_MSIX_TEST_PFX -and $env:SIGNTOOL_RS_MSIX_TIMESTAMP_URL -and $env:SIGNTOOL_RS_MSIX_DLIB -and $env:SIGNTOOL_RS_MSIX_DMDF) {
    ...
    $tempNativeMsixDecoupled = Join-Path $env:TEMP "signtool_rs_ci_msix_native_decoupled.msix"
    $tempRustMsixDecoupled = Join-Path $env:TEMP "signtool_rs_ci_msix_rust_decoupled.msix"
...
if ($winmdSrc -and $env:SIGNTOOL_RS_TEST_PFX -and (Test-Path -LiteralPath $winmdSrc)) {
    ...
    $tmpWinmdNat = Join-Path $env:TEMP "signtool_rs_winmd_native.winmd"
    $tmpWinmdRust = Join-Path $env:TEMP "signtool_rs_winmd_rust.winmd"
    ...
    $winmdParityDesc = "signtool_rs_winmd_parity_desc_2026"
    $winmdParityUrl = "https://example.invalid/signtool-rs-winmd-parity"
    $tmpWinmdDescNat = Join-Path $env:TEMP "signtool_rs_winmd_desc_native.winmd"
    $tmpWinmdDescRust = Join-Path $env:TEMP "signtool_rs_winmd_desc_rust.winmd"
...
if ($mmsiSrc -and $env:SIGNTOOL_RS_TEST_PFX -and (Test-Path -LiteralPath $mmsiSrc)) {
    ...
    $tmpMsiNat = Join-Path $env:TEMP "signtool_rs_msi_native.msi"
    $tmpMsiRust = Join-Path $env:TEMP "signtool_rs_msi_rust.msi"
    ...
    $msiParityDesc = "signtool_rs_parity_desc_2026"
    $msiParityUrl = "https://example.invalid/signtool-rs-parity"
    $tmpMsiDescNat = Join-Path $env:TEMP "signtool_rs_msi_desc_native.msi"
    $tmpMsiDescRust = Join-Path $env:TEMP "signtool_rs_msi_desc_rust.msi"
