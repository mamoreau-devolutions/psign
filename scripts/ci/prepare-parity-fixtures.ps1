...
$tempCopy = Join-Path $env:TEMP "signtool_rs_parity_ci_signed.exe"
...
$contentPath = Join-Path $env:TEMP "signtool_rs_parity_detached_content.bin"
Copy-Item -LiteralPath $UnsignedPe -Destination $contentPath -Force
$p7Dir = Join-Path $env:TEMP "signtool_rs_parity_p7_out"
